...
These can be used seamlessly regardless of underlying frameworks for above-mentioned steps.
...
Scenarios are configured using the files written JSON format files.
...
```bash
python -m kenning.scenarios.json_inference_tester scenario.json build/native.json --verbosity INFO
```
...
<!-- name="json-scenario-runner" -->
```bash
python -m kenning.scenarios.render_report build/native.json 'native' build/benchmarks/native.rst --root-dir build/benchmarks --img-dir build/benchmarks/imgs --verbosity INFO --report-types performance classification
```
...
The `build/benchmarks/native.rst` file is a ReStructuredText document containing a full report for the model - apart from linking to the generated visualizations, it provides aggregated information about CPU and memory usage, as well as classification quality metrics, such as accuracy, sensitivity, precision or G-Mean.
...
<!-- name="json-scenario-runner" -->
```bash
python -m kenning.scenarios.json_inference_tester scenario.json build/tflite-fp32.json --verbosity INFO

python -m kenning.scenarios.render_report build/tflite-fp32.json 'tflite-fp32' build/benchmarks/tflite-fp32.rst --root-dir build/benchmarks --img-dir build/benchmarks/imgs --verbosity INFO --report-types performance classification
```

While it depends on the used platform, there should be a significant improvement in both inference time (model ca. 10-15x faster model compared to the native model) and memory usage (output model ca. 2x smaller).
...
<!-- name="json-scenario-runner" -->
```bash
python -m kenning.scenarios.json_inference_tester scenario.json build/tflite-int8.json --verbosity INFO

python -m kenning.scenarios.render_report build/tflite-int8.json 'tflite-int8' build/benchmarks/tflite-int8.rst --root-dir build/benchmarks --img-dir build/benchmarks/imgs --verbosity INFO --report-types performance classification
```
...
To speed up inference of a quantized model, we can utilize vector extensions in the x86 CPUs, more specifically AVX2.
...
The `TVMCompiler`, with `llvm -mcpu=core-avx2` as the target optimizes and compiles the model to use vector extensions. The final result is a `.tar` file containing shared object that implements the entire model.

Let's compile the scenario (`tvm-avx2-int8.json`):

<!-- name="json-scenario-runner" -->
```bash
python -m kenning.scenarios.json_inference_tester scenario.json build/tvm-avx2-int8.json --verbosity INFO

python -m kenning.scenarios.render_report build/tvm-avx2-int8.json 'tvm-avx2-int8' build/benchmarks/tvm-avx2-int8.rst --root-dir build/benchmarks --img-dir build/benchmarks/imgs --verbosity INFO --report-types performance classification
```
...
The `MeasurementsCollector` class collects all benchmarks' data for the model inference and saves it in JSON format that can be later used to render reports with the `kenning.scenarios.render_report` script.
