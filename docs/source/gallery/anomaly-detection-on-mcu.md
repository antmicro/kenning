...
```bash
uv run kenning optimize --cfg ./kenning-scenarios/vae-inference/tflite-max32690.yml
```
...
```bash
uv run west build -p always -b max32690evkit/max32690/m4 app -- \
...
```bash
uv run kenning test --cfg ./kenning-scenarios/vae-inference/tflite-max32690.yml --measurements workspace/vae-tflite-renode.json --verbosity INFO
```
...
```bash
uv run kenning report --measurements workspace/vae-tflite-renode.json --report-path reports/vae-tflite-renode/report.md --to-html
```
...
```bash test-skip
uv run kenning test report --cfg ./kenning-scenarios/vae-inference/tflite-max32690.yml \
...
```bash
uv run kenning optimize --cfg ./kenning-scenarios/vae-inference/tvm-max32690.yml
```

Then compile the evaluation app using microTVM as the runtime.
Since microTVM requires building with the model, we provide path to the model and set `CONFIG_KENNING_TVM_MODEL_PRE_GEN=y` (since the model is pre-generated by Kenning).

```bash
uv run west build -p always -b max32690evkit/max32690/m4 app -- -DEXTRA_CONF_FILE='tvm.conf;boards/max32690evkit_max32690_m4.conf' -DCONFIG_KENNING_MODEL_PATH=\"$(realpath ./workspace/vae_cats.graph_data)\" -DCONFIG_KENNING_TVM_MODEL_PRE_GEN=y
uv run west build -t board-repl
```
...
```bash
uv run kenning test report --cfg ./kenning-scenarios/vae-inference/tvm-max32690.yml \
...
```bash
uv run kenning report --measurements \
