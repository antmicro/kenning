...
```bash
uv pip install "kenning[iree,tvm,torch,anomaly_detection,tensorflow,tflite,reports,renode,uart,zephyr,auto_pytorch] @ git+https://github.com/antmicro/kenning.git"
```
:::
...
```bash
west build -p always -b max32690evkit/max32690/m4 app -- -DEXTRA_CONF_FILE='tvm.conf;boards/max32690evkit_max32690_m4.conf' -DCONFIG_KENNING_MODEL_PATH=\"$(realpath ./workspace/vae_cats.graph_data)\" -DCONFIG_KENNING_TVM_MODEL_PRE_GEN=y
west build -t board-repl
```
