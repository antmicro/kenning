...
This chapter describes [Kenning Zephyr Runtime](https://github.com/antmicro/kenning-zephyr-runtime) - a set of tools for deploying and evaluating models optimized with Kenning on [Zephyr RTOS](https://www.zephyrproject.org/).

Kenning Zephyr Runtime is a C library that consists of:

* `kenning_inference_lib` provides an implementation of [Kenning Protocol](kenning-protocol) (for communication with Kenning) and [Kenning Runtime](runtime-api)-like unified API allowing easy preparation and execution of ML models with the following frameworks:
...
* `app` is a server-like application using `kenning_inference_lib` to evaluate models with Kenning running on host using [Kenning Protocol](kenning-protocol) to send and receive models, outputs and runtime statistics
* `demo_app` is an example application, showcasing how `kenning_inference_lib` can be used without Kenning as a library to run model inference on production regardless of underlying framework
...
Using uv with Kenning Zephyr Runtime is encouraged for fast dependency installation.
...
You can also install Kenning manually, for a different selection of additional dependencies.
For example:
...
However in this example we will use the [Renode](https://renode.io/) to simulate the board.
...
* Optimize the model with `TFLiteCompiler`, creating `./output/magic-wand.tflite` model.
* Automatically run Renode simulation of the built app, finding ELF in `zephyr_build_path` directory (`simulated: true` option means that Kenning will run Renode simulation automatically).
* Connect over (virtual) UART and send model and data for evaluation to the simulated board.
...
Zephelin allows for collecting traces either through an UART port, USB port or with GDB debugger.
...
:::{note}
Kenning Zephyr Runtime also sends logs through Kenning Protocol, aside from printing them.
Therefore after printing logs to the UART console is disabled, Kenning will still receive and display them.
:::
...
To do that, run normal build first.
For example:
