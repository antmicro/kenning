...
def get_edge_ai_tester_submodule_from_path(modulepath: str):
    """
    Converts script path to edge_ai_tester submodule name.
    ...
    modulename = '.'.join(parts[parts.index('edge_ai_tester'):]).rstrip('.py')
    return modulename
...
def get_command(argv: List[str]):
    ...
    modulename = get_edge_ai_tester_submodule_from_path(command[0])
    flagpresent = False
