...
class DatasetModelRegistry:
    ...
    def get(cls, framework: str) -> Tuple[Dataset, ModelWrapper, str]:
        ...
        if framework == "keras":
            ...
            # Deleting the ModelWrapper's function for saving io_spec, to
            # prevent our io_spec from being overwritten further down the line.
            # That's because our ModelWrapper is actually meant for TFLite, so
            # it would improperly overwrite iospec generated by IREE compiler.
