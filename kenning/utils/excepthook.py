...
def kenning_missing_import_excepthook(
    ...
    if (type_ == ModuleNotFoundError and last_tb.tb_frame.f_globals
            .get('__package__', '').startswith('kenning.')):
...
def find_missing_optional_dependency(module_name: str) -> Optional[str]:
    ...
    for extra, modules in dependencies.items():
        ...
        if any(
            _normalize(_get_pypi_name(module_name)) in
            _normalize(re.match(RE_NAME, name).group(0))
            for name in modules if not re.match(RE_KENNING_DEP, name)
        ):
            found = extra
            break
